...
- **Primer is the default genome for every frame on every surface** — pin `--genome brutalist` to keep the previous default look.

### Added

- **`--spec` and `--spec-file` on both commands** — `compose` and `validate` each take a spec as inline JSON (`--spec`), a file path (`--spec-file`), or a bundled preset name (`--spec-file gateway`).
- **Diagram worked example** — `discover verbs` teaches the compose → extract → mutate → recompose loop; every printed call runs as-is.

### Fixed

- **Validate agrees with compose** — same genome defaults, dotted `genome.variant`, receipt fallback, and refusal messages on CLI, HTTP, and MCP; a spec that validates always composes.
- **Clean errors everywhere** — a rejected spec lists each field path (`edges[0].source: Field required`), unknown genomes and variants name the fix, and nothing prints a traceback — including a `--spec-file` name that collides with a directory.
- **Mermaid-shaped input works** — edges accept `from`/`to`, unnamed nodes get `n0`, `n1`, … ids; payloads keep the canonical names.
- **Promoted topology recorded** — a cyclic `dag` rendered as `state-machine` writes `rendered.topology` into the payload.

### Notes

- Promoted cyclic-dag artifact ids differ from v0.4.0; all other ids are unchanged.
- Unknown-variant failures report 404 in `x-hw-error-code`; `worked_example` is now keyed per frame.
...
- **Discover on every surface** — `hyperweave discover` and `GET /v1/discover` join the MCP tool; hints run as printed, a pasted `what=` still answers, and an unknown selector errors with the menu.
...
- **Verify checks both proofs** — `verify` reports `well_formed` beside the hash check; exit 1 means the content was altered, exit 3 means the file won't parse as XML.
...
- **Errors name the problem** — a malformed spec prints the violated rule, a hub edge names the topology to recompose with, and an unknown top-level compose field lists the legal set instead of silently composing a default.
- **Transforms keep the look** — transforming an artifact keeps its background and color mode instead of resetting them.
...
- **Charts say why data is missing** — rate limited (with the upstream's retry hint, including GitHub's hourly reset), repo not found, or a token scope problem; fetches retry briefly and a hanging upstream is cut off at ten seconds.
- **Overlapping arrows part** — an arrow entering and an arrow leaving the same side of a node no longer sit on top of each other.
- **Help keeps its annotations** — bracketed notes render in `compose --help`.
- **Takeoff divider flames render everywhere** — they animate by scale and opacity, never geometry morphing.
