...
Production reliability hardening — Tier 1 (config-only) of a two-step rollout addressing the README scatter-loading symptom directly observed in Fly.io Grafana: 192 MiB sustained on a 256 MiB cap (75% memory pressure with no burst headroom), CPU spiking to 100% on the shared-cpu-1x slice, p99 response time hitting 2.08 minutes during burst windows, machine restarts (yellow instance restarted twice in a 1h window) wiping the in-process connector cache, and `/health` TTFB at 11.5s on cold visit because `auto_stop_machines = "stop"` paid a full Python interpreter cold start on every wake. v0.2.11-v0.2.16 code is correct; the cumulative architecture had zero margin for any deploy or refetch storm. Tier 2 (singleton httpx.AsyncClient + FastAPI lifespan warmup + module-scope imports + readiness-gating /health + pytest-asyncio fixture) ships in v0.2.18 after Tier 1 verifies in production. Sequencing rationale: Tier 1 alone eliminates an estimated 80% of the symptom and is a clean rollback target if Tier 2 surfaces any unexpected issue.

### Fixed

- **fly.toml `auto_stop_machines = "stop"` → `"suspend"`.** The dominant cause of the /health TTFB at 11.5s observed via external probing — every machine wake paid a full Python interpreter + FastAPI bootstrap + lazy-import cascade (~5-15s) before serving the first byte. Suspend preserves process state across idle so wake is ~1-2s. With `min_machines_running = 1` this yields one warm-state machine, immediate response on wake, and no cache wipe between idle cycles.
- **fly.toml `[[vm]] memory = "256mb"` → `"512mb"`.** Grafana directly observed 192 MiB sustained on the 256 MiB cap with ~64 MiB headroom — insufficient for burst response buffers (~80 KB × 30 README images fanning out concurrently through Camo) plus the Pydantic dump and Jinja2 context allocations on each compose. Doubling memory eliminates the OOM-pressure GC-thrashing pattern that produced the "Run 2 slower than Run 1" signature in sequential external probing.
- **fly.toml `[http_service.concurrency]` added with `soft_limit=20, hard_limit=40`.** App Concurrency in Grafana peaked at ~33 during README burst windows; the hard limit gates Fly autoscale before the running machine OOMs while the soft limit starts queueing earlier so tail latency stays bounded. Without backpressure, p99 response time correlated 1:1 with concurrency spikes, climbing to 2.08 minutes when ~33 simultaneous requests hit the throttled CPU slice.
- **`config/settings.py` cache TTL split by route role.** Pre-existing `data_cache_ttl=300` was applied uniformly to pure-compose routes (badge static-state, icon, divider, strip without `?data=`) AND data-bound routes. Static-compose artifacts have no upstream input but `max-age=300` forced Camo to refetch them every 5 minutes against an origin with a cold connector cache — turning visible static content into a cold-start lottery despite the artifacts being cacheable indefinitely. Three new fields:
  - `compose_cache_ttl = 86400` for pure-compose routes; Camo refetches once daily (only when HyperWeave version ships)
  - `data_cache_ttl = 300` unchanged, paired with `stale-while-revalidate=3600` so Camo can serve stale data while async-revalidating
  - `error_cache_ttl = 5` paired with `stale-while-revalidate=60` for SMPTE error fallbacks (was hardcoded `max-age=60`). Aggressive so a recovered origin re-populates Camo within seconds rather than the previous minute — short cold-start outages no longer amplify into minute-long broken-image cascades for every README visitor.
- **`serve/app.py` 8 Cache-Control header sites updated.** `_compose_and_respond` (304 + 200 paths) now read `compose_cache_ttl`. Six error-fallback sites (4 inline `?data=` parse errors + 2 catch-all exception handlers) consolidated into a new `_error_response_headers(status_code)` helper that reads `error_cache_ttl + stale-while-revalidate=60` so the TTL is configurable via `HW_ERROR_CACHE_TTL` env var without source edits.

### Notes

- **Diagnostic provenance.** Symptom converged from three independent surfaces: (a) local codebase audit identified the architectural patterns (per-request `httpx.AsyncClient`, 30 lazy imports inside async handlers, 5-min TTL on static-compose routes); (b) external probing from a clean network path measured /health TTFB 11.5s, marquee 12-30s, p99 2.08 minutes, Run-2-slower-than-Run-1; (c) Fly.io Grafana dashboard directly observed memory pressure (192/256 MiB sustained), machine restart cycles, CPU 100% spikes, App Concurrency peaks (~33), HTTP Response Times pathology (p99=2.08min correlated with concurrency spikes), and 5min Load Avg hitting 100%+. The Grafana evidence elevated the diagnosis from confident inference to directly observed pathology and validated the concurrency limits before they shipped.
- **Tier 2 plan.** `/Users/k01101011/.claude/plans/hey-im-sending-velvet-island.md` captures the full architectural fix that ships in v0.2.18: singleton `httpx.AsyncClient` with HTTP/2 multiplexing + connection pool (eliminates the 5-fresh-TLS-handshake cost on the marquee fan-out), FastAPI lifespan warmup (force-loads compose pipeline so first traffic doesn't pay cold-import cost), module-scope hyperweave imports (resolves Python's import-lock serialization), 503-returning `/health` exercising the compose pipeline (gates Fly's readiness probe on actual readiness not just port-bind), pytest-asyncio autouse fixture for httpx-singleton test isolation. Three rounds of cross-review (local Explore agents + parallel Claude UI external probing + four Fly.io Grafana captures) signed off before Tier 1 staging.
- **Post-deploy success signal.** Watch HTTP Response Times in Grafana during a README burst. p99 should stay under 2 seconds (was 2.08 minutes pre-deploy). If p99 still climbs into seconds-or-minutes, Tier 1 didn't fully resolve the symptom and Tier 2 needs investigation before shipping.
