"""Surface pack — target-aware post-processing of an assembled SVG.

A composed artifact is `var(--dna-*)`-based and animated by default (the ``web``
target). Off-web rasterizers (CairoSVG, email clients, PDF converters) do not
resolve CSS custom properties — paints collapse to black/transparent (BUG-002) —
and many surfaces want no motion. Each ``--target`` is a list of pure string
passes over the final SVG; ``web`` is the identity (byte-stable).
...
def resolve_vars_to_hex(svg: str) -> str:
    ...
    ink — never to empty (which is the black/transparent collapse BUG-002 is about).
    """
...
def clamp_width(svg: str, max_w: int = 800) -> str:
    """Cap the rendered width (slack's 800px ceiling); viewBox preserves aspect."""
...
# target → ordered passes. ``web`` is the identity (byte-stable vs today).
_TREATMENTS: dict[str, list[str]] = {
    "web": [],
    "github": ["vars"],  # GitHub renders CSS but dark-mode contrast is safer flattened
    "email": ["vars", "noanim"],
    "pdf": ["vars", "noanim"],
    "obsidian": ["vars"],
    "slack": ["vars", "noanim", "clamp"],
}
...
def apply_target(svg: str, target: str) -> str:
    """Run the surface-pack passes for ``target`` over the assembled SVG."""
    for name in _TREATMENTS.get(target, []):
