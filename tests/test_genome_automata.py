...
def test_automata_bifamily_chromatic_fields_populated() -> None:
    ...
    # Blue family
    assert len(spec.family_blue_rim_stops) == 7
    assert len(spec.family_blue_pattern_cells) == 3
    assert spec.family_blue_label_slab_fill == "#0A1C28"
    assert spec.family_blue_value_text == "#A8D4F0"
    # Purple family
    assert len(spec.family_purple_rim_stops) == 7
    assert len(spec.family_purple_pattern_cells) == 3
    assert spec.family_purple_label_slab_fill == "#150A22"
    assert spec.family_purple_value_text == "#D8B4FE"
    # Bifamily bridge (divider palette)
    assert spec.bifamily_bridge_teal_mid == "#147A90"
    assert spec.bifamily_bridge_amethyst_core == "#5A3278"
    # Pulse config
...
def test_state_palette_backfilled_across_shipped_genomes() -> None:
    ...
    genomes = get_genome_specs()
    for slug in ("automata", "brutalist-emerald", "chrome-horizon", "telemetry-void"):
        spec = genomes[slug]
...
def test_every_shipped_genome_still_passes_validation_after_backfill() -> None:
    ...
    for brutalist-emerald/chrome-horizon/telemetry-void."""
    paradigms = get_paradigms()
